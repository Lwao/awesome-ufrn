\documentclass[letterpaper, 10 pt, conference]{ieeeconf}  
\IEEEoverridecommandlockouts                              
\overrideIEEEmargins

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{float}
\usepackage{comment}
\usepackage{hyperref}
\PassOptionsToPackage{hyphens}{url}

\usepackage{url}
\makeatletter
\g@addto@macro{\UrlBreaks}{\UrlOrds}

\input{style}

\title{\LARGE \bf
OFDM signal generation with IFFT}

\author{Levy G. da S. Galvão$^{1}$ e Thiago M. Souto$^{2}$% <-this % stops a space
\thanks{*This work was not supported by any organization}% <-this % stops a space
\thanks{$^{1}$ Electrical engineering undergraduate, Universidade Federal do Rio Grande do Norte, Brazil.}%
\thanks{$^{2}$ Electrical engineering undergraduate, Universidade Federal do Rio Grande do Norte, Brazil.}%
}

\begin{document}

\maketitle
\thispagestyle{empty}
\pagestyle{empty}

\input{body/abstract}
\input{body/introduction}
\input{body/development}
\input{body/results}
\input{body/conclusions}
\input{body/biblyography}




\addtolength{\textheight}{-12cm}  

\begin{comment}
\section*{APPENDIX}

Appendixes should appear before the acknowledgment.

\section*{ACKNOWLEDGMENT}

The preferred spelling of the word ``acknowledgment'' in America is without an ``e'' after the ``g''. Avoid the stilted expression, ``One of us (R. B. G.) thanks . . .''  Instead, try ``R. B. G. thanks''. Put sponsor acknowledgments in the unnumbered footnote on the first page.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

References are important to the reader; therefore, each citation must be complete and correct. If at all possible, references should be commonly available publications.

\end{comment}






\end{document}
